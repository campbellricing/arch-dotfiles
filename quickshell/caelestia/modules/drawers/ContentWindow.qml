...
StyledWindow {
    ...
    HyprlandFocusGrab {
        ...
        active: (visibilities.launcher && root.contentItem.Config.launcher.enabled) || (visibilities.session && root.contentItem.Config.session.enabled) || (visibilities.sidebar && root.contentItem.Config.sidebar.enabled) || (visibilities.utilities && root.contentItem.Config.utilities.enabled) || (!root.contentItem.Config.dashboard.showOnHover && visibilities.dashboard && root.contentItem.Config.dashboard.enabled) || (panels.popouts.currentName.startsWith("traymenu") && (panels.popouts.current as StackView)?.depth > 1) || (panels.popouts.hasCurrent && !panels.popouts.isDetached)
        windows: [root]
        onCleared: {
            visibilities.launcher = false;
            visibilities.session = false;
