...
CustomMouseArea {
    ...
    // Open on hover is hardcoded upstream for these two panels; set to true to restore it
    ...
    onPositionChanged: event => {
        ...
        if (panels.sidebar.offsetScale === 1) {
            // Show osd on hover
            const showOsd = osdShowOnHover && inRightPanel(panels.osdWrapper, x, y);

            // Always update visibility based on hover if not in shortcut mode
            if (osdShowOnHover && !osdShortcutActive) {
            ...
            // Show osd on hover
            const showOsd = osdShowOnHover && outOfSidebar && inRightPanel(panels.osdWrapper, x, y);
        ...
        const showUtilities = utilitiesShowOnHover && inBottomPanel(panels.utilities, x, y, true);

        // Always update visibility based on hover if not in shortcut mode
        if (utilitiesShowOnHover && !utilitiesShortcutActive) {
